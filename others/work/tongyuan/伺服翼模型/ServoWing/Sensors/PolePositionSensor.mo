...
model PolePositionSensor "磁极位置传感器"
  //参数
  parameter Integer n = 12 "采集器位数";
  parameter SI.Length min = 0 "最小输出位移";
  parameter SI.Length max = 12 "最大输出位移";
...
equation 
  connect(flange_a, positionSensor.flange)
    annotation (Line(origin = {-80.0, 0.0}, 
      ...
      color = {0, 127, 0}));
  connect(positionSensor.s, a_d.u)
    annotation (Line(origin = {-29.0, 0.0}, 
      points = {{-26.0, 0.0}, {16.0, 0.0}}, 
      color = {0, 0, 127}));
  connect(a_d.y, s)
    annotation (Line(origin = {65.0, 0.0}, 
      points = {{-56.0, 0.0}, {45.0, 0.0}}, 
      color = {0, 0, 127}));
end PolePositionSensor;