...
model PMSMControllerX "永磁同步电机控制器_喷管角度作为反馈"
  extends Icons.BasicModel;
  //参数
  parameter SI.Time T_PWM = 1e-4 "PWM波周期";
  parameter SI.Voltage U_dc = 220 "额定电压";
  parameter Real p = 2 "电机极对数";
  final parameter Real theata_offset = 0;

  parameter Real Kp1 = 2.607 "比例系数,V/A" 
    annotation (Dialog(tab = "电流环参数"));
  parameter Real Ki1 = 0 "积分系数" 
    annotation (Dialog(tab = "电流环参数"));
  parameter Real Kd1 = 0 "微分系数" 
    annotation (Dialog(tab = "电流环参数"));
  parameter Real u_max1 = 220 "d轴,q轴电压指令限幅,V" 
    annotation (Dialog(tab = "电流环参数"));
  parameter SI.Time T1 = 1e-5 "电流环控制周期" 
    annotation (Dialog(tab = "电流环参数"));

  parameter Real Kp2 = 0.456 "比例系数,A/(rad/s)" 
    annotation (Dialog(tab = "角速度环参数"));
  parameter Real Ki2 = 0.01 "积分系数" 
    annotation (Dialog(tab = "角速度环参数"));
  parameter Real Kd2 = 0 "微分系数" 
    annotation (Dialog(tab = "角速度环参数"));
  parameter Real u_max2 = 120 "交轴电流指令限幅,A" 
    annotation (Dialog(tab = "角速度环参数"));
  parameter SI.Time T2 = 1e-4 "角速度环控制周期" 
    annotation (Dialog(tab = "角速度环参数"));

  parameter Real Kp3 = 2142 * (180 / pi) "比例系数,(rad/s)/rad" 
    annotation (Dialog(tab = "位置环参数"));
  parameter Real Ki3 = 0 "积分系数" 
    annotation (Dialog(tab = "位置环参数"));
  parameter Real Kd3 = 0 "微分系数" 
    annotation (Dialog(tab = "位置环参数"));
  parameter Real u_max3 = 18000 * (2 * pi / 60) "角速度指令限幅,rad/s" 
    annotation (Dialog(tab = "位置环参数"));
  parameter Real u_max4 = 8.5 * (pi / 180) "角度输入限幅，rad" 
    annotation (Dialog(tab = "位置环参数"));

  parameter SI.Time T3 = 0.001 "位置环控制周期" 
    annotation (Dialog(tab = "位置环参数"));
  ...
  Modelica.Blocks.Nonlinear.Limiter limiter3(uMax = u_max2)
    annotation (Placement(transformation(origin = {-20.0, -50.0}, 
      extent = {{-10.0, -10.0}, {10.0, 10.0}})));
  Modelica.Blocks.Sources.RealExpression realExpression(y = if IsUsePosLoop then phi_pg else 0)
  ...
  Modelica.Blocks.Interfaces.RealInput w "角速度反馈,rad/s" 
    annotation (Placement(transformation(origin = {-76.545, 111.335}, 
      ...
      iconTransformation(origin = {-110.0, 96.0}, 
        extent = {{-10.0, -10.0}, {10.0, 10.0}}, 
        rotation = 360.0)));
  Modelica.Blocks.Interfaces.RealInput phi_pg "角度反馈,rad" 
    annotation (Placement(transformation(origin = {19.455, 109.335}, 
      extent = {{-10.0, -10.0}, {10.0, 10.0}}, 
      rotation = -90.0), 
      iconTransformation(origin = {-110.0, 130.0}, 
        extent = {{-10.0, -10.0}, {10.0, 10.0}}, 
        rotation = 360.0)));
  Modelica.Blocks.Nonlinear.Limiter limiter4(uMax = u_max4)
    annotation (Placement(transformation(origin = {-100.0, -112.0}, 
      extent = {{-10.0, -10.0}, {10.0, 10.0}})));
equation 
  ...
  annotation (
    Icon(coordinateSystem(extent = {{-100.0, -100.0}, {100.0, 100.0}}, 
      ...
      extent = {{-15.0, 10.0}, {15.0, -10.0}}, 
      textString = "w", 
      textStyle = {TextStyle.None}), Text(origin = {-73.5, 133.0}, 
      extent = {{-23.5, 11.0}, {23.5, -11.0}}, 
      textString = "phi_pg", 
...
false-速度闭环，true-位置闭环", 
      ...
      extent = {{-19.0, 3.0}, {19.0, -3.0}}, 
      textString = "角速度反馈,rad/s", 
      textStyle = {TextStyle.None}), Text(origin = {21.0, 121.0}, 
      extent = {{-19.0, 3.0}, {19.0, -3.0}}, 
      textString = "喷管角度反馈,rad", 
      textStyle = {TextStyle.None})}));
  ...
  connect(w, feedback2.u2)
    annotation (Line(origin = {-64.0, 63.0}, 
      ...
      color = {0, 0, 127}));
  connect(phi_ref, limiter4.u)
    annotation (Line(origin = {-121.0, -96.0}, 
      points = {{-9.0, 15.0}, {1.0, 15.0}, {1.0, -16.0}, {9.0, -16.0}}, 
      color = {0, 0, 127}));
  connect(limiter4.y, feedback4.u1)
    annotation (Line(origin = {-83.0, -112.0}, 
