...
model PlanarWorld "平面世界坐标系"
  ...
  parameter Boolean axisShowLabels = true "= true, if labels shall be shown" 
    annotation (Dialog(tab = "Animation", group = "if animateWorld = true", enable = enableAnimation and animateWorld));
  parameter MB.Types.Color axisColor_x = MB.Types.Defaults.FrameColor
    "Color of x-arrow" 
  ...
  parameter SI.Diameter gravityArrowDiameter = gravityArrowLength / 
    defaultWidthFraction "Diameter of gravity arrow" annotation (Dialog(tab = 
      ...
      enable = enableAnimation and animateGravity and gravityType == GravityTypes.UniformGravity));
  parameter MB.Types.Color gravityArrowColor = {0, 180, 0} "Color of gravity arrow" annotation (HideResult = true, Dialog(colorSelector = true, tab = "Animation", group = "if animateWorld = true", enable = enableAnimation and animateWorld));
...
protected 
  ...
  ServoWing.Mechanics.Components.Visualizers.Advanced.Arrows gravityArrow(
    r = zeros(3), 
