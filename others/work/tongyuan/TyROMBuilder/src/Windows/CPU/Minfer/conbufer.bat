...
:: 循环遍历AIModel.mo文件修改参数
(for /f "delims=" %%a in (%CONBUFER_BASEPATH%/template/AIModel.mo) do (
:: 将一个值赋给一个变量
set "str=%%a"
...
echo,!str!
endlocal
))>"%CONBUFER_BASEPATH%/tool/AIModel.mo"

:: 循环遍历callC.mo文件修改参数
(for /f "delims=" %%a in (%CONBUFER_BASEPATH%/template/callC.mo) do (
:: 将一个值赋给一个变量
set "str=%%a"
setlocal enabledelayedexpansion
:: 旧字符串=新字符串
...
echo,!str!
endlocal
))>"%CONBUFER_BASEPATH%/tool/callC.mo"

:: 创建模板
cd %CONBUFER_BASEPATH%
xcopy /q/y/s "build" "%MODEL_PATH%/Minfer/build" >nul
xcopy /q/y/s "infer" "%MODEL_PATH%/Minfer/infer" >nul
xcopy /q/y/s "tool" "%MODEL_PATH%/Minfer/tool" >nul
echo GENERATE TEMPLATE SUCCESS.

::删除中间过程文件
cd %CONBUFER_BASEPATH%/tool
if exist AIModel.mo (del /q AIModel.mo)
if exist callC.mo (del /q callC.mo)
