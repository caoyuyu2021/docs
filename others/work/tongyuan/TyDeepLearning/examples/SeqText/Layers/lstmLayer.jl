...
# 创建一个LSTM层,输入大小为10，隐藏层大小16，网络层数为2
#创建一个数组，使用该层处理数组并输出数组维度
InputSize = 10
numHiddenUnits = 16
layer = lstmLayer(InputSize, numHiddenUnits; NumLayers=2, Batch_First=true)
...
layers = SequentialCell([
    lstmLayer(inputSize, numHiddenUnits),
    fullyConnectedLayer(numHiddenUnits, numClasses),
    softmaxLayer()
])
